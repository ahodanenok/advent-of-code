...
public class Day11 {
    ...
    private static void part1(Map<Point, Integer> octopuses) {
        int flashesCount = 0;
        for (int i = 0; i < 100; i++) {
            flashesCount += flash(octopuses).size();
        }

        System.out.println("Part 1: " + flashesCount);
    }

    private static void part2(Map<Point, Integer> octopuses) {
        int step = 1;
        while (flash(octopuses).size() != octopuses.size()) {
            step++;
        }
    ...
    private static Set<Point> flash(Map<Point, Integer> octopuses) {
